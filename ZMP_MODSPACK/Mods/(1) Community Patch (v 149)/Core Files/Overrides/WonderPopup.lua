...
function OnPopup( popupInfo )
	...
	-- Game Info
	local strGameInfo = GetHelpTextForBuilding(iBuildingID, true, true, true, nil, true, false, true)

    
    if( strGameInfo ~= nil ) then
        Controls.Stats:SetText( Locale.ConvertTextKey( strGameInfo ) );
        Controls.StatsScroll:CalculateInternalSize();
    	Controls.StatsScroll:SetHide( false );
    else
    	Controls.StatsScroll:SetHide( true );
    end
