...
--	FILE:	 Rainforest.lua
--	AUTHOR:  Bob Thomas
--	PURPOSE: Global map script - A world almost entirely covered by jungle.
------------------------------------------------------------------------------
...
function GetMapScriptInfo()
	...
	return {
		Name = "TXT_KEY_MAP_RAINFOREST_VP",
		Description = "TXT_KEY_MAP_RAINFOREST_HELP",
		IsAdvancedMap = 0,
		IconIndex = 1,
		SortIndex = 1,
		CustomOptions = {
...
function PangaeaFractalWorld:GeneratePlotTypes()
	...
	-- Reject the result until the largest landmass occupies 84% or more of the total land.
	local done = false;
	local iAttempts = 0;
...
function GeneratePlotTypes()
	print("Generating Plot Types (Lua Rainforest) ...");
	...
	if userInputLandmass == 5 then -- Random
		userInputLandmass = 1 + Map.Rand(4, "Rainforest Random Landmass Type - Lua");
	end
...
function GenerateTerrain()
	print("Generating Terrain (Lua Rainforest) ...");
...
------------------------------------------------------------------------------
function FeatureGenerator:AddJunglesAtPlot(plot, iX, iY, _)
...
function AddFeatures()
	print("Adding Features (Lua Rainforest) ...");
...
function StartPlotSystem()
	...
	print("Choosing start locations for civilizations - Rainforest Lua.");
	args = {};
